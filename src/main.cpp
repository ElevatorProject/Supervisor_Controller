...
int main() {
    ...
    while (1) {
        ...
        case 1:
            ID =
                chooseID(); // user to select ID depending on intended recipient
            data = chooseMsg(); // user to select message data
            pcanTx(ID, data);   // transmit ID and data
            db_setFloorNum(FloorFromHex(
                data)); // change floor number in database ** NEW **
            break;
        ...
        case 3:
            printf("\nNow listening to commands from the website - press "
                   "ctrl-z to "
                   "cancel\n");
            ...
            while (1) {
                ...
                if (prev_floorNumber !=
                    floorNumber) { // If floor number changes in database
                    pcanTx(ID_SC_TO_EC,
                           HexFromFloor(floorNumber)); // change floor number
                                                       // in elevator - send
                                                       // command over CAN
                }
                prev_floorNumber = floorNumber;
                sleep(1); // poll database once every second to check for change
                          // in
                          // floor number
            }
            break;
